...
async function init() {
    ...
    document.getElementById('copyLinkBtn').addEventListener('click', () => {
        copyToClipboard(window.location.origin + '/lobby?join=' + roomId + '&game=pegsolitaire');
        showCopyToast('Invite link copied!');
    });

    connection.on('PegSolitaireRoomUpdated', room => renderRoom(room));
    connection.on('PegSolitaireRoomDissolved', () => { alert('The room was closed.'); window.location.href = '/lobby'; });
    connection.on('KickedFromRoom', () => { alert('You were kicked.'); window.location.href = '/lobby'; });
    connection.on('PegSolitaireGameStarted', room => {
        sessionStorage.setItem('pegSolitaireRoomId', room.id);
...
function renderRoom(room) {
    ...
    document.getElementById('roomTitle').textContent = esc(room.settings.roomName || 'Peg Solitaire Room');
...
function _renderPlayerList(room) {
    ...
    if (isHost) {
        ...
        if (!canStart) btn.title = 'Need at least 2 players to start';
...
document.getElementById('startBtn').addEventListener('click', () => connection.invoke('StartPegSolitaireGame', roomId));
function kickPlayer(name) { connection.invoke('KickPegSolitairePlayer', roomId, name); }

document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('backBtn').addEventListener('click', () =>
        connection.invoke('LeavePegSolitaireRoom', roomId).then(() => { window.location.href = '/lobby'; }));
});
...
function showCopyToast(msg) {
    ...
    clearTimeout(t._t);
    t._t = setTimeout(() => t.classList.remove('copy-toast-show'), 2500);
}
