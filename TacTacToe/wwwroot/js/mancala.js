...
function _spawnParticles(count, originX, originY, speedScale, canvas, particles) {
    for (let i = 0; i < count; i++) {
        const angle = Math.random() * Math.PI * 2;
        const speed = (2 + Math.random() * 9) * speedScale;
        particles.push({ x: originX, y: originY, vx: Math.cos(angle)*speed*(0.6+Math.random()), vy: Math.sin(angle)*speed*-1.2 - Math.random()*5*speedScale, size: 4+Math.random()*6, color: _confColors[Math.floor(Math.random()*_confColors.length)], rotation: Math.random()*360, rotSpeed: (Math.random()-0.5)*14, shape: Math.random()<0.4?'circle':Math.random()<0.7?'rect':'strip', opacity: 1, gravity: 0.11+Math.random()*0.09, drag: 0.98+Math.random()*0.015, wobble: Math.random()*Math.PI*2, wobbleSpeed: 0.03+Math.random()*0.06, fadeStart: 160 });
    }
}
function _runConfettiCanvas(canvas, particles, maxFrames) {
    const ctx = canvas.getContext('2d'); let w = canvas.width = window.innerWidth, h = canvas.height = window.innerHeight, frame = 0;
    function draw() {
        ctx.clearRect(0,0,w,h); let alive = false;
        for (const p of particles) { p.vy += p.gravity; p.vx *= p.drag; p.vy *= p.drag; p.x += p.vx + Math.sin(p.wobble)*1.5; p.y += p.vy; p.rotation += p.rotSpeed; p.wobble += p.wobbleSpeed; if (frame>p.fadeStart) p.opacity -= 0.016; if (p.opacity<=0) continue; alive=true; ctx.save(); ctx.translate(p.x,p.y); ctx.rotate(p.rotation*Math.PI/180); ctx.globalAlpha=Math.max(0,p.opacity); ctx.fillStyle=p.color; if(p.shape==='circle'){ctx.beginPath();ctx.arc(0,0,p.size/2,0,Math.PI*2);ctx.fill();}else if(p.shape==='rect')ctx.fillRect(-p.size/2,-p.size/2,p.size,p.size*0.6);else ctx.fillRect(-p.size/2,-1,p.size,2.5); ctx.restore(); }
        frame++; if (alive && frame<maxFrames) requestAnimationFrame(draw); else canvas.remove();
    }
    requestAnimationFrame(draw);
}
function launchConfetti() {
    const canvas = document.createElement('canvas'); canvas.style.cssText='position:fixed;inset:0;z-index:9999;pointer-events:none;'; document.body.appendChild(canvas);
    const w = canvas.width = window.innerWidth, h = canvas.height = window.innerHeight, particles = [];
    _spawnParticles(220, w/2+(Math.random()-0.5)*w*0.3, h*0.45, 1.1, canvas, particles);
    setTimeout(()=>_spawnParticles(100,w*0.15,h*0.35,0.9,canvas,particles),250);
    setTimeout(()=>_spawnParticles(100,w*0.85,h*0.35,0.9,canvas,particles),400);
...
async function init() {
    ...
    connection.on("MancalaUpdated", state => {
        const prev = gameState;
        gameState = state;
