...
async function init() {
    ...
    connection.on("MancalaRoomUpdated", room => renderRoom(room));
    connection.on("MancalaRoomDissolved", () => { alert("The room was dissolved."); window.location.href = "/lobby"; });
    connection.on("MancalaGameStarted", () => { window.location.href = "/mancala"; });
    ...
    document.getElementById("copyLinkBtn").addEventListener("click", () => {
        navigator.clipboard.writeText(window.location.origin + "/lobby?join=" + roomId + "&game=mancala");
    });
    document.getElementById("startBtn").addEventListener("click", () => {
        connection.invoke("StartMancalaGame", roomId);
    });
...
function renderRoom(room) {
    ...
    document.getElementById("roomTitle").textContent = room.settings?.roomName || "Mancala Room";
