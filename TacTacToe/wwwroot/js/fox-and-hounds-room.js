...
if (!roomId) {
    ...
    throw new Error('Missing Fox and Hounds room id');
}
...
async function init() {
    ...
    document.getElementById('copyLinkBtn').onclick = () => {
        copyToClipboard(window.location.origin + '/lobby?join=' + roomId + '&game=fox-and-hounds');
        showCopyToast('Invite link copied!');
    };

    connection.on('FoxAndHoundsRoomUpdated', renderRoom);
    connection.on('FoxAndHoundsRoomDissolved', () => { alert('The room was closed.'); window.location.href = '/lobby'; });
    connection.on('KickedFromRoom', () => { alert('You were kicked.'); window.location.href = '/lobby'; });
    connection.on('FoxAndHoundsGameStarted', () => {
...
function renderRoom(room) {
    ...
    document.getElementById('roomTitle').textContent = room.settings.roomName || 'Fox and Hounds Room';
...
document.getElementById('startBtn').addEventListener('click', () => {
    ...
    connection.invoke('StartFoxAndHoundsGame', roomId, fillBots);
});
...
function kickPlayer(name) { connection.invoke('KickFoxAndHoundsPlayer', roomId, name); }

document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('backBtn').addEventListener('click', () =>
        connection.invoke('LeaveFoxAndHoundsRoom', roomId).then(() => { window.location.href = '/lobby'; }));
});
