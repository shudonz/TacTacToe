...
app.MapGet("/bones", (HttpContext ctx) =>
{
    if (ctx.User.Identity?.IsAuthenticated != true)
        return Results.Redirect("/login");
    return Results.File("bones.html", "text/html");
});

app.MapGet("/bones-room", (HttpContext ctx) =>
{
    if (ctx.User.Identity?.IsAuthenticated != true)
        return Results.Redirect("/login");
...
app.MapGet("/mancala", (HttpContext ctx) =>
{
    if (ctx.User.Identity?.IsAuthenticated != true)
        return Results.Redirect("/login");
    return Results.File("mancala.html", "text/html");
});

app.MapGet("/mancala-room", (HttpContext ctx) =>
{
    if (ctx.User.Identity?.IsAuthenticated != true)
        return Results.Redirect("/login");
    return Results.File("mancala-room.html", "text/html");
});
...
app.MapGet("/connect-sum", (HttpContext ctx) =>
{
    if (ctx.User.Identity?.IsAuthenticated != true)
        return Results.Redirect("/login");
    return Results.File("connect-sum.html", "text/html");
});

app.MapGet("/connect-sum-room", (HttpContext ctx) =>
{
    if (ctx.User.Identity?.IsAuthenticated != true)
        return Results.Redirect("/login");
    return Results.File("connect-sum-room.html", "text/html");
});
